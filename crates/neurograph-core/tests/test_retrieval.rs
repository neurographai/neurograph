...
async fn setup_test_graph() -> (Arc<MemoryDriver>, Arc<HashEmbedder>) {
    ...
    let rel1 = Relationship::new(
        ...
        "WORKS_AT", "Alice works at Anthropic as a researcher",
    ).with_group_id("test");

    let rel2 = Relationship::new(
        bob.id.clone(), anthropic.id.clone(),
        "FOUNDED", "Bob founded Anthropic in 2021",
    ).with_group_id("test");

    let rel3 = Relationship::new(
        anthropic.id.clone(), sf.id.clone(),
        "LOCATED_IN", "Anthropic is located in San Francisco",
    ).with_group_id("test");
...
async fn test_semantic_search_by_text() {
    ...
    let results = SemanticSearcher::search(
        "Alice", 5, None,
        embedder.as_ref(), driver.as_ref(),
    ).await.unwrap();
...
async fn test_semantic_search_by_vector() {
    ...
    let results = SemanticSearcher::search_by_vector(
        &embedding, 5, None, driver.as_ref(),
    ).await.unwrap();
...
async fn test_keyword_search() {
    ...
    let results = KeywordSearcher::search(
        "Alice", 5, None, driver.as_ref(),
    ).await.unwrap();
...
async fn test_keyword_search_no_results() {
    ...
    let results = KeywordSearcher::search(
        "nonexistent_entity_xyz_12345", 5, None, driver.as_ref(),
    ).await.unwrap();

    assert!(results.is_empty(), "Should find no results for nonexistent query");
}
...
async fn test_hybrid_search() {
    ...
    let results = retriever
        .search(
            "Alice researcher", 5, None, Some("test"),
            embedder.as_ref(), driver.as_ref(),
        )
        .await
        .unwrap();
...
async fn test_hybrid_search_custom_weights() {
    ...
    let results = retriever
        .search("Anthropic", 3, None, None, embedder.as_ref(), driver.as_ref())
        .await
        .unwrap();
...
async fn test_hybrid_search_with_seeds() {
    ...
    if let Some(alice) = search_results.first() {
        ...
        let results = retriever
            .search("researcher", 5, Some(&seed_ids), None, embedder.as_ref(), driver.as_ref())
            .await
            .unwrap();

        // With seeds, traversal should also contribute
        assert!(!results.is_empty(), "Should find results with seed-based traversal");
    }
}
...
async fn test_recipe_find_entity() {
    ...
    let entity = SearchRecipes::find_entity(
        "Anthropic", embedder.as_ref(), driver.as_ref(),
    ).await;
...
async fn test_recipe_find_related() {
    ...
    let results = SearchRecipes::find_related(
        "AI safety", 5, embedder.as_ref(), driver.as_ref(),
    ).await;
...
fn test_context_builder_with_entities_and_relationships() {
    ...
    assert!(ctx.context_text.contains("Alice"), "Context should mention Alice");
    assert!(ctx.context_text.contains("Anthropic"), "Context should mention Anthropic");
    ...
    let entities: Vec<Entity> = (0..50)
        .map(|i| {
            Entity::new(format!("Entity{}", i), "Test")
                .with_summary("A very long summary that takes up lots of tokens in the context window")
        })
        .collect();

    let ctx = builder.build(&entities, &[], &[], "test");
    assert!(ctx.entity_count < 50, "Should truncate at budget: got {}", ctx.entity_count);
}
...
fn test_query_classification_global() {
    let router = QueryRouter::new();
    assert_eq!(router.classify("Summarize all the topics"), QueryType::Global);
    assert_eq!(router.classify("Give me an overview of everything"), QueryType::Global);
    assert_eq!(router.classify("What are the main themes?"), QueryType::Global);
}
...
fn test_query_classification_temporal() {
    ...
    assert_eq!(router.classify("What was true before 2020?"), QueryType::Temporal);
...
fn test_query_classification_multihop() {
    let router = QueryRouter::new();
    assert_eq!(router.classify("How is Alice connected to Bob?"), QueryType::MultiHop);
    assert_eq!(router.classify("What is the relationship between X and Y?"), QueryType::MultiHop);
}
...
fn test_budget_can_afford() {
    ...
    assert!(!budget.can_afford(0.05), "Should not afford after spending most of budget");
}
...
async fn test_query_engine_local() {
    ...
    // Ingest some data
    ng.add_text("Alice works at Anthropic as a researcher").await.unwrap();
    ...
    assert!(
        result.answer.contains("Alice") || result.answer.contains("Anthropic") || result.answer.contains("No relevant"),
