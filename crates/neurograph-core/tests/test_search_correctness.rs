...
async fn test_vector_search_nearest_neighbor() {
    ...
    let ml = Entity::new("Machine Learning", "Concept")
        .with_embedding(vec![1.0, 0.0, 0.0]);
    let dl = Entity::new("Deep Learning", "Concept")
        .with_embedding(vec![0.9, 0.1, 0.0]);
    let cooking = Entity::new("Cooking", "Concept")
...
async fn test_vector_search_returns_k_results() {
    ...
    for i in 0..20 {
        let e = Entity::new(&format!("Entity_{}", i), "Test")
            .with_embedding(vec![i as f32 / 20.0, 0.5, 0.5]);
...
async fn test_text_search_exact_name_match() {
    ...
    driver.store_entity(&Entity::new("Alice Smith", "Person")
        .with_summary("A researcher")).await.unwrap();
    driver.store_entity(&Entity::new("Bob Jones", "Person")
        .with_summary("A chef")).await.unwrap();
...
async fn test_text_search_summary_match() {
    ...
    driver.store_entity(&Entity::new("Alice", "Person")
        .with_summary("A researcher at Anthropic working on AI safety")).await.unwrap();
    driver.store_entity(&Entity::new("Bob", "Person")
        .with_summary("A chef in New York")).await.unwrap();
...
async fn test_text_search_case_insensitive() {
    ...
    driver.store_entity(&Entity::new("UPPERCASE", "Test")
        .with_summary("ALL CAPS ENTITY")).await.unwrap();
...
async fn test_traversal_single_hop() {
    ...
    driver.store_relationship(&Relationship::new(
        alice.id.clone(), bob.id.clone(), "KNOWS", "Alice knows Bob",
    )).await.unwrap();
    driver.store_relationship(&Relationship::new(
        bob.id.clone(), charlie.id.clone(), "KNOWS", "Bob knows Charlie",
    )).await.unwrap();
...
async fn test_traversal_two_hops() {
    ...
    driver.store_relationship(&Relationship::new(
        a.id.clone(), b.id.clone(), "L", "AB",
    )).await.unwrap();
    driver.store_relationship(&Relationship::new(
        b.id.clone(), c.id.clone(), "L", "BC",
    )).await.unwrap();
...
async fn test_traversal_handles_cycles() {
    ...
    driver.store_relationship(&Relationship::new(c.id.clone(), a.id.clone(), "L", "CA")).await.unwrap();
...
async fn test_ng_search_entities() {
    ...
    ng.store_entity(&Entity::new("Rust Programming", "Concept")).await.unwrap();
    ng.store_entity(&Entity::new("Python Programming", "Concept")).await.unwrap();
    ng.store_entity(&Entity::new("Cooking Recipes", "Concept")).await.unwrap();
